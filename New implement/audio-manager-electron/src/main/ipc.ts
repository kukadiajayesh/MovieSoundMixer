...
import { validateInputFile, validateOutputPath, resolveOutputPath } from './files/fileManager'
...
export function setupIPCHandlers(mainWindow: BrowserWindow) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const args = ['-y', '-i', inputPath, '-map', `0:a:${streamIdx - 1}`]
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const mkvPath = getMkvmergePath()
      const useMkvMerge = outContainer === 'mkv' && mkvPath !== null
      ...
      if (useMkvMerge) {
        ...
        if (copyVideo) {
          ...
          args.push(...getGPUEncoderArgs(encoder ?? 'libx264', 'balanced'))
        }
